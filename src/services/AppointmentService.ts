...
export class AppointmentService {
    ...
    public async listAppointmentByDate(doctorId: number, date: string): Promise<string> {
    ...
    public async addAppointment(doctorId: number, firstName: string, lastName: string, date: string, time: string, kind: any): Promise<string> {
        // validate time
        const mins = parseInt(time.split(":")[1].substring(0, 2));
        if(mins % 15 !== 0) {
            throw new Error(`Invalid time provided, only intervals of 15 are allowed`);
        }
    ...
    public async deleteAppointment(doctorId: number, appointmentId: number): Promise<string> {
