...
export class AppointmentRepository {
    ...
    public async listByDate(doctorId: number, date: string): Promise<Appointment[]> {
        if(!this.appMap.has(doctorId)) {
            throw new Error(`Doctor with id ${doctorId} not found`);
        }
    ...
    public async add(doctorId: number, appointment: Appointment): Promise<boolean> {
    ...
    public async delete(doctorId: number, appointmentId: number): Promise<boolean> {
        if(!this.appMap.has(doctorId)) {
            throw new Error(`Doctor with id ${doctorId} not found`);
        }
        _.remove(this.appMap.get(doctorId), (appointment: Appointment) => appointment.id === appointmentId);
        return true;
    }
}